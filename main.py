...
def main():
    ...
    def plot_histograms(metrics_dict, metric_name):
        plt.figure(figsize=(12, 6))
        model_names = list(metrics_dict.keys())
        metric_values = {model: metrics[metric_name] for model, metrics in metrics_dict.items()}
    
        for model_name in model_names:
            plt.hist(metric_values[model_name], alpha=0.5, label=model_name, bins=10)

        plt.xlabel(metric_name)
        plt.ylabel('Frequency')
        plt.title(f'Histogram of {metric_name} for Models')
        plt.legend()
        plt.grid(True)
        plt.show()

# Assuming you have stored metrics for each model in a dictionary
    models_metrics = {'CNN': cnn_metrics, 'RNN': rnn_metrics, 'LSTM': lstm_metrics}

# Plot histograms for accuracy, precision, recall, and F1 score
    metrics_to_plot = ['accuracy', 'precision', 'recall', 'f1_score']
    for model_name, metrics in models_metrics.items():
        print(f"Model: {model_name}, Available Metrics: {metrics.keys()}")


# Define a function to plot histograms for metrics
